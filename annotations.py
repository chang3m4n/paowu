...
#   默认指向根目录下的VOC数据集
# -------------------------------------------------------#
VOCdevkit_path = 'trainingdata/12'
...
if __name__ == "__main__":
    ...
    if annotation_mode == 0 or annotation_mode == 2:
        print("Generate train.txt and test.txt for train.")
        type_index = 0
        for image_set in VOCdevkit_sets:
            image_ids = open(os.path.join(VOCdevkit_path, 'ImageSets/%s.txt' % image_set),
                             encoding='utf-8').read().strip().split()
            list_file = open('%s.txt' % image_set, 'a', encoding='utf-8')
            ...
            type_index += 1
            list_file.close()
        print("Generate train.txt and test.txt for train done.")
